...
/**
 * Gasless Send Client for ICTT Contracts
 * 
 * Usage:
 * 1. With command line argument:
 *    node gasless_send_client.js 0x8a22131111b07684e5df85ef89ffd5350ce106115207c818a3bba12470744eb4
 * 
 * 2. With environment variable (Windows):
 *    set RECIPIENT_PRIVATE_KEY=0x8a22131111b07684e5df85ef89ffd5350ce106115207c818a3bba12470744eb4
 *    node gasless_send_client.js
 * 
 * 3. With environment variable (Unix/Linux/Mac):
 *    export RECIPIENT_PRIVATE_KEY=0x8a22131111b07684e5df85ef89ffd5350ce106115207c818a3bba12470744eb4
 *    node gasless_send_client.js
 * 
 * 4. With default private key (for testing only):
 *    node gasless_send_client.js
 */
...
class GaslessSendClient {
    ...
    async testRegularSend(wallet, contracts, recipient, amount) {
        ...
        // Step 1: Approve ERC20Remote contract to spend user's tokens
        console.log('Step 1: Approving ERC20Remote contract to spend user tokens...');
        
        // Check current allowance first
        const currentAllowance = await erc20Remote.allowance(wallet.address, erc20Remote.target);
        console.log(`Current allowance: ${ethers.formatUnits(currentAllowance, 6)} USDC`);
        
        // Approve the contract to spend the user's tokens (not the other way around)
        // Use a larger approval amount to ensure we have enough
        const approvalAmount = amount * 2n; // Approve double the amount
        const approveTx = await erc20Remote.approve(erc20Remote.target, approvalAmount);
        await approveTx.wait();
        console.log(`✓ Approved ${ethers.formatUnits(approvalAmount, 6)} USDC for ERC20Remote contract`);
        console.log(`  Transaction hash: ${approveTx.hash}`);
        
        // Verify the allowance was set correctly
        const newAllowance = await erc20Remote.allowance(wallet.address, erc20Remote.target);
        console.log(`New allowance: ${ethers.formatUnits(newAllowance, 6)} USDC`);
        
        // Check user's balance
        const userBalance = await erc20Remote.balanceOf(wallet.address);
...
function loadConfig(deploymentResultsPath = null) {
    ...
    // Try multiple possible locations for the deployment results
    const possiblePaths = [
        deploymentResultsPath, // User-provided path
        path.join(projectRoot, 'scripts', 'deployment_gasless_results.json'), // New location in scripts
        path.join(projectRoot, 'contracts', 'ictt', 'deployment_gasless_results.json'), // Old location
    ...
    if (!configPath) {
        ...
        throw new Error(`Deployment results not found. Searched in: ${searchedPaths}`);
    }
...
async function runGaslessSendTests(recipientPrivateKey = null) {
    try {
        ...
        // Test parameters - try multiple sources for recipient private key
        const recipientPrivateKeyForGasless = recipientPrivateKey || 
                                             process.env.RECIPIENT_PRIVATE_KEY || 
                                             process.env.PRIVATE_KEY || 
                                             '0x8a22131111b07684e5df85ef89ffd5350ce106115207c818a3bba12470744eb4';
        ...
        const homeToRemoteAmount = ethers.parseUnits('0.2', 6); // 0.2 USDC (6 decimals) - initial transfer to remote
        const gaslessAmount = ethers.parseUnits('0.1', 6); // 0.1 USDC (6 decimals) - gasless return transfer
        const regularAmount = ethers.parseUnits('0.1', 6); // 0.1 USDC (6 decimals) - regular return transfer
        
        console.log('\n=== Starting Gasless Send Tests ===');
        console.log(`Test recipient: ${recipient}`);
        console.log(`Home to Remote amount: ${ethers.formatUnits(homeToRemoteAmount, 6)} USDC`);
        console.log(`Gasless return amount: ${ethers.formatUnits(gaslessAmount, 6)} USDC`);
        console.log(`Regular return amount: ${ethers.formatUnits(regularAmount, 6)} USDC`);
        ...
        while (recipientBalance < homeToRemoteAmount && retryCount < maxRetries) {
            ...
            if (recipientBalance < homeToRemoteAmount) {
        ...
        // Check if user has enough tokens for the regular send test
        const userBalanceForRegularSend = await recipientContracts.erc20Remote.balanceOf(recipientWallet.address);
        console.log(`User balance for regular send: ${ethers.formatUnits(userBalanceForRegularSend, 6)} USDC`);
        
        let regularSendReceipt;
        if (userBalanceForRegularSend < regularAmount) {
            console.log(`⚠️ Skipping regular send test - insufficient balance. Need ${ethers.formatUnits(regularAmount, 6)} USDC, have ${ethers.formatUnits(userBalanceForRegularSend, 6)} USDC`);
            console.log('This is expected since tokens were consumed in the gasless send test.');
            
            // Create a mock receipt for the skipped test
            regularSendReceipt = {
                success: false,
                skipped: true,
                reason: 'Insufficient balance after gasless send'
            };
        } else {
            // Use a different recipient for the regular send test (properly checksummed)
            const differentRecipient = '0x742d35cc6634c0532925a3b8d4c9db96c4b4d8b6'; // Different address
            regularSendReceipt = await client.testRegularSend(
                recipientWallet, 
                recipientContracts, 
                differentRecipient, // Send to different address for testing
                regularAmount
            );
        }
        ...
        const testResults = {
            ...
            testConfiguration: {
                recipient: recipient,
                homeToRemoteAmount: ethers.formatUnits(homeToRemoteAmount, 6),
                gaslessAmount: ethers.formatUnits(gaslessAmount, 6),
                regularAmount: ethers.formatUnits(regularAmount, 6),
...
if (require.main === module) {
    ...
    console.log(`Running gasless send tests`);
    if (recipientPrivateKey) {
        console.log(`Using recipient private key from command line argument`);
    } else if (process.env.RECIPIENT_PRIVATE_KEY) {
        console.log(`Using recipient private key from RECIPIENT_PRIVATE_KEY environment variable`);
    } else if (process.env.PRIVATE_KEY) {
        console.log(`Using recipient private key from PRIVATE_KEY environment variable`);
    } else {
        console.log(`Using default recipient private key`);
    }
